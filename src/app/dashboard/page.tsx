...
export default function Page() {
    ...
    useEffect(() => {
        ...
        const fetchDocuments = async () => {
            try {
                const res = await fetch(
                    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/documents/getAllDocuments/${user.id}`
                );
                ...
                <div className="w-full max-w-7xl py-5 px-4 sm:px-6 lg:px-0">
                    ...
                    {isLoadingDocs ? (
                        <div className="w-full h-[400px] flex items-center justify-center">
                            <MoonLoader color="#7752FE" size={24} speedMultiplier={1.5} />
                        </div>
...
function DocumentCard({ onCreate }: { onCreate: () => void }) {
    return (
        ...
        >
            ...
            <p className="font-medium cursor-default select-none text-left">
                Blank Document
            </p>
        </button>
    );
